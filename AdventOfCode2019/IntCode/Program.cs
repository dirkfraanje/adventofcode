...
namespace IntCode
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while(true)
            {
                SetValuesAndPositions(startIndex);
                    ...
                    switch (opCode)
                {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        Console.WriteLine("Diagnostics code: " + program[parameter1]);
                        Console.ReadLine();
                        startIndex += 2;
                        break;
                    case 5:
                        startIndex = program[parameter1] != 0 ? Convert.ToInt32(program[parameter2]) : startIndex + 3;
                        break;
                    case 6:
                        startIndex = program[parameter1] == 0 ? Convert.ToInt32(program[parameter2]) : startIndex + 3;
                        break;
                    ...
                    case 8:
                        ...
                        break;
                    case 9:
                        relativeBase += Convert.ToInt32(program[parameter1]);
                        startIndex += 2;
                        break;
                    case 99:
                        Console.WriteLine("OpCode99. Value at position 0 = " + program[0]);
                        Console.ReadLine();
                        return;
                    default:
                        throw new Exception("This is not a valid opcode");
                }
            }
        }

        private static void SetValuesAndPositions(int startIndex)
        {
            var valueFromStartIndex = program[startIndex].ToString();
            switch (valueFromStartIndex.Length)
            {
                case 1:
                case 2:
                    opCode = Convert.ToInt32(program[startIndex]);
                    parameter1 = Convert.ToInt32(program[startIndex + 1]);
                    parameter2 = opCodeWith2Parameters(opCode) ? 0 : Convert.ToInt32(program[startIndex + 2]);
                    parameter3 = opCodeWith2Parameters(opCode) ? 0 : Convert.ToInt32(program[startIndex + 3]);
                    break;
                case 3:
                    opCode = Int32.Parse(valueFromStartIndex.Substring(1));
                    parameter1Mode = Int32.Parse(valueFromStartIndex.Substring(0, 1));
                    parameter1 = parameter1Mode == 0 ? Convert.ToInt32(program[startIndex + 1]) : parameter1Mode == 1 ? startIndex + 1 : Convert.ToInt32(relativeBase + program[startIndex + 1]);
                    parameter2 = opCodeWith2Parameters(opCode) ? 0 : Convert.ToInt32(program[startIndex + 2]);
                    parameter3 = opCodeWith2Parameters(opCode) ? 0 : Convert.ToInt32(program[startIndex + 3]);
                    break;
                case 4:
                    opCode = Int32.Parse(valueFromStartIndex.Substring(2));
                    parameter1Mode = Int32.Parse(valueFromStartIndex.Substring(1, 1));
                    parameter2Mode = Int32.Parse(valueFromStartIndex.Substring(0, 1));
                    parameter1 = parameter1Mode == 0 ? Convert.ToInt32(program[startIndex + 1]) : parameter1Mode == 1 ? startIndex + 1 : Convert.ToInt32(relativeBase + program[startIndex + 1]);
                    parameter2 = parameter2Mode == 0 ? Convert.ToInt32(program[startIndex + 2]) : parameter2Mode == 1 ? startIndex + 2 : Convert.ToInt32(relativeBase + program[startIndex + 2]);
                    parameter3 = opCodeWith2Parameters(opCode) ? 0 : Convert.ToInt32(program[startIndex + 3]);
                    break;
                case 5:
                    opCode = Int32.Parse(valueFromStartIndex.Substring(3));
                    parameter1Mode = Int32.Parse(valueFromStartIndex.Substring(2, 1));
                    parameter2Mode = Int32.Parse(valueFromStartIndex.Substring(1, 1));
                    parameter3Mode = Int32.Parse(valueFromStartIndex.Substring(0, 1));
                    parameter1 = parameter1Mode == 0 ? Convert.ToInt32(program[startIndex + 1]) : parameter1Mode == 1 ? startIndex + 1 : Convert.ToInt32(relativeBase + program[startIndex + 1]);
                    parameter2 =  parameter2Mode == 0 ? Convert.ToInt32(program[startIndex + 2]) : parameter2Mode == 1 ? startIndex + 2 : Convert.ToInt32(relativeBase + program[startIndex + 2]);
                    parameter3 = parameter3Mode == 0 ? Convert.ToInt32(program[startIndex + 3]) : parameter3Mode == 1 ? startIndex + 3 : Convert.ToInt32(relativeBase + program[startIndex + 3]);
                    break;
                default:
                    break;
            }
        ...
        private static bool opCodeWith2Parameters(int opCode)
        {
            return opCode == 3 || opCode == 4 || opCode == 9 ? true : false;
        }
    }
}
