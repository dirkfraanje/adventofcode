...
namespace AdventOfCode2020.Day2
{
    ...
    {
        static List<string> passwords = new List<string>() { "6-7 w: wwhmzwtwwk",
        ...
        public static void Execute()
        {
            ...
            foreach (var password in passwords)
            {
                ...
                var letterNeeded = policy.Substring(length - 1).ToCharArray()[0];
                var digits = policy.Substring(0, length - 2).Split('-');
                var digit1 = int.Parse(digits[0]);
                var digit2 = int.Parse(digits[1]);
                bool valid = Validate(digit1, digit2, letterNeeded, splitted[1]);
                if (valid)
            ...
            timer.Stop();
            Console.WriteLine();
            Console.WriteLine($"Answer: {countValidPasswords} valid passwords");
            Console.WriteLine($"Executed in: {timer.ElapsedMilliseconds} milliseconds ({timer.ElapsedTicks}  Ticks)");
        }

        private static bool Validate(int digit1, int digit2, char letterNeeded, string password)
        {
            var character1 = password[digit1];
            var character2 = password[digit2];
            if (character1 == letterNeeded && character2 != letterNeeded)
                return true;
            if (character1 != letterNeeded && character2 == letterNeeded)
                return true;
            return false;
